...
It has been a long time since I've done a [TidyTuesdsay](https://github.com/rfordatascience/tidytuesday) post, but wind turbines, time series, and maps are hard to resist! (Especially on a snowy shoulder season vacation!) Because this dataset is relatively small, my goal was to make an interactive dashboard using crosstalk contained in a standalone web page.

My first step was to grab the data and make it into a shared data object accessible to crosstalk. My overall goal was to present an option for looking at the data across time, but immediately I discovered the `commissioning_date` field would need a bit of clean up. Who has a column of the form `year1/year2/year...`? 😞 Unfortunately there didn't appear to be an easy way to fully determine the actual commission date. A couple of options presented themselves:
...
# pre-process then create crosstalk object
wind<- wind_turbine %>% 
  ...
  mutate(computed_year = paste0(computed_year,"-01-01"),
         ...
         year = year(computed_year)) %>%  # useful for plotting
  mutate(label = paste0(
    project_name, " <br/>", 
...
One final note. As you can see in the map, there is a bit of overplotting... One way to handle this normally in plotly is to use `clusterIds`, which I demonstrate below. Unfortunately [at this time](https://github.com/rstudio/leaflet/issues/478) clustering is not supported with crosstalk filters.
