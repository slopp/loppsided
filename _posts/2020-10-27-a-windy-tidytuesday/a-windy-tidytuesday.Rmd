...
You can learn a bit about this week's dataset here, but essentially we have spatio-temporal information on wind Canadian turbines. My overall goal was to present an option for looking at the data across time, but immediately I discovered the "time" component of our dataset -  `commissioning_date` - bit of clean up. Who has a column where some values are `2001` and some are `2001/2002`? 😞  Unfortunately there didn't appear to be an easy way to fully determine the actual commission date. A couple of options presented themselves:
