...
- October-Nov: Ramping within Looker, which is still mostly independent.  
- Nov-Dec: Running the Looker SE playbook.  
- Jan: Re-org. Looker integrated with GCP, I land in a SMB COE. 
- Jan-Mar: Running the Looker SE playbook, but with new team.  
- Apr-June: Paternity leave.  
- June - Oct: Moved to enterprise GCP as a regional Looker specialist. Mostly running the GCP SE playbook.
...
Looker ran an incredibly tight playbook that was a great example of value oriented selling. They also built ramp, tools, and processes that executed that playbook. The Looker process included:  
...
Google Cloud was an entirely different beast. I was a data analytics specialist, an overlay to the GCP account teams who sold the entire GCP portfolio. (Actually I was specialist which was an overlay to the DA overlay). If Looker was about value selling, GCP was about solution selling which meant:  
...
This motion was sometimes done with greenfield accounts but was often done within existing GCP accounts. There were two goals: to secure a commit and then to turn that commit into revenue by driving workload consumption.
...
In addition to the sales motion I got exposed to GCP's take on IAM, cloud consoles, and their crown jewel: BigQuery.
...
So what's next? I'll be joining Elementl as their first SE. I am excited to be back at a small startup where the playbooks, pitch decks, and demos still need to be built. I'm also really excited about the [technology](https://github.com/slopp/snowreport). At Google I enjoyed getting more hands on with SQL running Looker plays. The GCP playbook, on the other hand, covers so many tools it's impossible to have more than a shallow understanding of each. I'm looking forward to being back in the trenches selling, but also writing Python and wrangling Kubernetes. Elementl is a good fit for me because it is a technical sale to technical personas, with a small sharp team, built around a bottom's up motion.

I am grateful to have "double-dipped" and learned from both Looker and GCP. I am also grateful for a slower year with time off to learn how to be a father. While it is hard to leave the security and comfort and food of Google, for now I am looking forward to going back to startup "time". Maybe I'll return to the behemoth one day, but till then, thanks fellow Googlers!